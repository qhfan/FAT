...
from utils import load_checkpoint, load_pretrained2, save_checkpoint, get_grad_norm, auto_resume_helper, reduce_tensor, save_best_checkpoint, save_last_checkpoint
...
def main(config):
    ...
    if config.MODEL.PRETRAINED and (not config.MODEL.RESUME):
        max_accuracy = load_pretrained2(config, model_without_ddp, optimizer, lr_scheduler, logger)
        acc1, acc5, loss = validate(config, data_loader_val, model)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1:.1f}%")
        if config.EVAL_MODE:
            return
